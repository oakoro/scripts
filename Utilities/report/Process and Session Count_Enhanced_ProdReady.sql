...
from cte_sessionlogsize2 where name = ''' +@sessionlogtable +'''
...
select p.name AS ''ProcessName'', COUNT(l.sessionnumber) AS ''SessionLogCount'' from BPASession s with (nolock) 
	join '+@sessionlogtable +' l with (nolock) on s.sessionnumber = l.sessionnumber
...
order by s.SessionLogCount desc'
)