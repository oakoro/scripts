declare @aasp_create_New_Sessionlog_partition sysname,
		...
		@aasp_create_new_partition_BPASessionlog sysname, @aasp_copyDataDelta sysname, @aasp_getCopyDatesDelta sysname,
		@aasp_Update_adf_watermark_delta sysname
...
select @aasp_copyDataDelta = convert(nvarchar(20),UpdatedDate,20)  from @check where CheckName = 'aasp_copyDataDelta'
select @aasp_getCopyDatesDelta = convert(nvarchar(20),UpdatedDate,20)  from @check where CheckName = 'aasp_getCopyDatesDelta'
select @aasp_Update_adf_watermark_delta = convert(nvarchar(20),UpdatedDate,20)  from @check where CheckName = 'aasp_Update_adf_watermark_delta'
...
select  isnull(@aasp_Update_adf_watermark_WQI,'Not Deployed')'aasp_Update_adf_watermark_WQI',
		...
		isnull(@aasp_copyDataDelta,'Not Deployed')'aasp_copyDataDelta',
		isnull(@aasp_getCopyDatesDelta,'Not Deployed')'aasp_getCopyDatesDelta',
		isnull(@aasp_Update_adf_watermark_delta,'Not Deployed')'aasp_Update_adf_watermark_delta'
