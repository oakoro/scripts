...
DECLARE @loggingType BIT, @sessionLogTable NVARCHAR(30),@tableExists BIT, @partitionExists BIT,@emptyTable BIT,
		...
		@nonclusterindexpartition NVARCHAR(max),@sessionlogindex NVARCHAR(100)
...
--Run Test
--select @sessionlogtable'@sessionlogtable'
...
--WHERE OBJECT_NAME(object_id) = @sessionlogtable)
--SET @partitionExists = 0
--ELSE SET @partitionExists = 1
ELSE
BEGIN
SET @tableExists = 1 
SET @emptyTable = 1
...
SELECT @emptyTable '@emptyTable'
----Run Test
--select @sessionlogtable'@sessionlogtable',@tableExists'@tableExists',@partitionExists'@partitionExists'

--IF @partitionExists = 0 AND @emptyTable = 0
...
/*STEP1_Identify dependants views and drop */
		...
		DECLARE @vsessionlogtable NVARCHAR(30)
		...
		--PRINT (@identifyViewStr)
		INSERT @schema_bound_views
		EXEC (@identifyViewStr)

		--SELECT * FROM @schema_bound_views
...
--Run Test
--select @sessionlogtable'@sessionlogtable',@tableExists'@tableExists',@partitionExists'@partitionExists',@vsessionlogtable'@vsessionlogtable',
--		@loggingType'@loggingType'

SET @CopySessionLogTable = @sessionlogtable+'Copy'
IF @loggingType = 0
...
--Run Test
select @sessionlogtable'@sessionlogtable',@tableExists'@tableExists',@partitionExists'@partitionExists',@vsessionlogtable'@vsessionlogtable',
		@loggingType'@loggingType',@CopySessionLogTable'@CopySessionLogTable'
/*STEP3_Create check constraints on copy session log table */
		...
		ADD CONSTRAINT cslogid CHECK (logid >= '+CONVERT(NVARCHAR(50),@mnidout) + ' and logid < ' +CONVERT(NVARCHAR(50),@mxidout) +') '
...
--Run Test
select @sessionlogtable'@sessionlogtable',@tableExists'@tableExists',@partitionExists'@partitionExists',@vsessionlogtable'@vsessionlogtable',
		@loggingType'@loggingType',@emptyTable '@emptyTable',@CopySessionLogTable'@CopySessionLogTable'
...
/*STEP6_Switch session log data back to production table*/
IF @emptyTable = 0
BEGIN 
