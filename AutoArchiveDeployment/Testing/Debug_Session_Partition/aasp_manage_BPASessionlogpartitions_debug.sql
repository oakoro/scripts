/****** Object:  StoredProcedure [BPC].[aasp_manage_BPASessionlogpartitions]    Script Date: 09/02/2024 17:24:05 ******/
SET ANSI_NULLS ON
GO
...
print 'EXECUTE [BPC].[aasp_create_New_Sessionlog_partition];'

-- Drop copied partition - Call [BPC].[aasp_delete_copied_Sessionlog_partition] 
print 'EXECUTE [BPC].[aasp_delete_copied_Sessionlog_partition] @partitionsretained = 5;'
END
ELSE
BEGIN
-- Partition session log table for the first time 
print 'EXECUTE [BPC].[aasp_create_new_partition_BPASessionlog];'
END
