/****** Object:  StoredProcedure [BPC].[aasp_create_new_partition_BPASessionlog]    Script Date: 09/02/2024 16:47:12 ******/
SET ANSI_NULLS ON
GO
...
-- =====================================================================================
-- Description: SP partitions unpartitioned session log table for the first time
-- Script PRINTution Example: BPC.aasp_create_new_partition_BPASessionlog
-- =======================================================================================
...
/*STEP5_Create partition function for empty partition table*/
BEGIN 
		PRINT '---Creating partition function and scheme---'
...
/*STEP7_Partition production session log table*/
BEGIN 
...
/*STEP8_Switch session log data back to production table and delete copy table*/
IF @emptyTable = 0
BEGIN 
		...
		PRINT (@deleteCopySessionLogTable)
PRINT '---Switching session log data back to production table and delete copy table completed---'
END

IF (SELECT TOP (1) ps.name AS [Partition Scheme] FROM sys.tables t INNER JOIN sys.indexes i ON t.object_id = i.object_id AND i.type IN (0,1) INNER JOIN sys.partition_schemes ps ON i.data_space_id = ps.data_space_id 
WHERE t.name in ('BPASessionLog_NonUnicode','BPASessionLog_Unicode')) = 'PS_Dynamic_NU'
BEGIN
