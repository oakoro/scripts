/****** Object:  StoredProcedure [BPC].[aasp_getCopyDatesDelta4]    Script Date: 20/11/2024 16:16:25 ******/
SET ANSI_NULLS ON
GO
...
-- =====================================================================================
-- @version 24.5.03
-- Description: Get copy date range
-- Script execution Example: EXEC [BPC].[aasp_getCopyDatesDelta4] 500,'BPAAuditEvents'
-- =======================================================================================

CREATE PROCEDURE [BPC].[aasp_getCopyDatesDelta4] 
@maxCount INT, @tablename VARCHAR(100)
AS

DECLARE @deltacol VARCHAR(100),@lastDateCopied DATETIME,@initCount INT,@currCount INT, @maxDate DATETIME, 
		@minDate DATETIME, @sql1 NVARCHAR(400),@sql2 NVARCHAR(400),@sql3 NVARCHAR(400),@sql4 NVARCHAR(400),
...
IF @lastDateCopied = @defaultdate
	...
	INSERT @finaltable VALUES(@minDate,ISNULL(@maxDate,@lastDateCopied),@tablename,@deltacol)
	...
	SELECT @currCount=currCount FROM @tblcurrCount
		...
		INSERT @finaltable VALUES(@minDate,@maxDate,@tablename,@deltacol)
		END
		ELSE
		BEGIN
		SET @minDate = @lastDateCopied
