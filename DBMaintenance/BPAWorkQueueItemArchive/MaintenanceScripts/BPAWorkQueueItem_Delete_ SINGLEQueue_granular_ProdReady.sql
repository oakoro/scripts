...
SET @Str = 'DELETE FROM dbo.BPAWorkQueueItem WHERE queueid = '''+CONVERT(NVARCHAR(50),@QueueToDelete) + ''''
	+' AND DATEPART(year,CAST(loaded AS DATE)) = ' +CONVERT(NVARCHAR(10),@Year) +' AND DATEPART(MONTH,CAST(loaded AS DATE)) = ' +CONVERT(NVARCHAR(10),@MONth)
	+' AND DATEPART(WEEK,CAST(loaded AS DATE)) = ' +CONVERT(NVARCHAR(10),@Week) +' AND DATEPART(DAY,CAST(loaded AS DATE)) = ' +CONVERT(NVARCHAR(10),@Day)
