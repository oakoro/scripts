...
where logid between 10305000 and 10310000
),
cte_2
as
(
...
--select cte_1.*,cte_2.rowsize,ROW_NUMBER() over (order by cte_1.logid) as rowNumber
--from cte_1 join cte_2 on cte_1.logid = cte_2.logid
,cte_3
as
(
select cte_1.*,cte_2.rowsize/1024/1024 as rowsize,ROW_NUMBER() over (order by cte_1.logid) as rowNumber
from cte_1 join cte_2 on cte_1.logid = cte_2.logid
) 
--cte_1.logid between 11800000 and 12533546
select logid, result,attributexml,rowsize,rowNumber from cte_3
where rowsize >= 20
group by logid, result,attributexml,rowsize,rowNumber
order by rowNumber 
