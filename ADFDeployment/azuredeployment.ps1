...
$dataFactorydeploymentctx = New-AzResourceGroupDeployment -ResourceGroupName RG-SQL -TemplateFile "$PSScriptRoot\datafactory_deployment.json" `
    -LS_ASQL_RPA_connectionString $RPAsqlDBconnectionSecure -LS_ASQL_BPC_connectionString  $BPCMaintenanceDBconnectionSecure `
    -dataFactory_properties_globalParameters_serverNameShort_value $sql.ServerName -LS_ADLS_Archive_properties_typeProperties_url $dlStorageURL.TrimEnd("/") `
    -dataFactory_location $sql.Location <#`
    -dataFactory_properties_globalParameters_subFolderSQLDBName_value $SQLDatabase `
    -dataFactory_properties_globalParameters_DatabaseName_value $SQLDatabase#>
