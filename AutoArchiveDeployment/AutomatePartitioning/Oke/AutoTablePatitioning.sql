...
IF NOT EXISTS(SELECT 1 FROM sys.tables where name = '''+@loggingTable+'TopLogid'')
BEGIN
CREATE TABLE [dbo].['+@loggingTable +'TopLogid](
	[logid] [bigint] IDENTITY(1,1) NOT NULL,
	[sessionnumber] [int] NOT NULL,
	[stageid] [uniqueidentifier] NULL,
	[stagename] [varchar](128) NULL,
	...
	[endtimezoneoffset] [int] NULL,
	[attributesize]  AS (isnull(datalength([attributexml]),(0))) PERSISTED NOT NULL,
 CONSTRAINT [PK_'+@loggingTable +'TopLogid] PRIMARY KEY CLUSTERED 
(
	[logid] ASC
)WITH (STATISTICS_NORECOMPUTE = OFF, IGNORE_DUP_KEY = OFF, OPTIMIZE_FOR_SEQUENTIAL_KEY = OFF) ON [PRIMARY]
) ON [PRIMARY] TEXTIMAGE_ON [PRIMARY];
...
ALTER TABLE [dbo].[' +@loggingTable +'TopLogid]  WITH CHECK ADD  CONSTRAINT [FK_'+@loggingTable +'TopLogid_BPASession] FOREIGN KEY([sessionnumber])
REFERENCES [dbo].[BPASession] ([sessionnumber]);


ALTER TABLE [dbo].[' +@loggingTable +'TopLogid] CHECK CONSTRAINT [FK_'+@loggingTable +'TopLogid_BPASession];



/****** Object:  Index [Index_BPASessionLog_NonUnicodeTopLogid_sessionnumber]    Script Date: 27/03/2023 10:43:01 ******/
CREATE NONCLUSTERED INDEX [Index_'+ @loggingTable +'TopLogid_sessionnumber] ON [dbo].[' +@loggingTable +'TopLogid]
(
	[sessionnumber] ASC
)WITH (STATISTICS_NORECOMPUTE = OFF, DROP_EXISTING = OFF, ONLINE = OFF, FILLFACTOR = 90, OPTIMIZE_FOR_SEQUENTIAL_KEY = OFF) ON [PRIMARY];
END
...
SET IDENTITY_INSERT [dbo].[' +@loggingTable +'TopLogid] ON;

INSERT INTO [dbo].[' +@loggingTable +'TopLogid]
(
            logid,
            sessionnumber,
            stageid,
 ...
 SELECT TOP 1 logid,
               ...
               endtimezoneoffset
        FROM
            [dbo].['+@loggingTable +'Copy] WITH (NOLOCK)
			ORDER BY LOGID DESC;

SET IDENTITY_INSERT [dbo].[' +@loggingTable +'TopLogid] OFF;

'
SET @deleteTopLogging = 'DELETE FROM [dbo].['+@loggingTable +'Copy] WHERE logid = (SELECT logid FROM [dbo].[' +@loggingTable +'TopLogid]);
END
'
...
PRINT (@CopySessionLogTbl)
PRINT (@switchCopySessionLog)
PRINT (@copyTopLogging)
PRINT (@deleteTopLogging)
END
--ELSE
--BEGIN
