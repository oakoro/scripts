declare @tablename varchar(100),@deltacol varchar(100),@lastDateCopied DATETIME,
		@currCount BIGINT, @maxDate DATETIME, @minDate DATETIME, @sql nvarchar(400),
		@maxCount BIGINT = 500, @sql2 nvarchar(400),@sql3 nvarchar(400)
...
for
select tablename, deltacolumn from BPC.adf_watermark where tablename in ('BPASessionLog_NonUnicode','BPASessionLog_Unicode')
...
IF @lastDateCopied = '1900-01-01 00:00:00.000'
	BEGIN
	SET @sql2 = 'SELECT TOP('+CONVERT(NVARCHAR(20),@maxCount) +') '+@deltacol+ ' FROM dbo.'+@tablename +' WITH (NOLOCK)  ORDER BY '+@deltacol 
...
--select @currCount = currCount,@minDate=minDate,@maxDate=maxDate FROM @hold;
select @tablename tablename  , @deltacol deltacol,@lastDateCopied lastDateCopied,@currCount currCount,@minDate minDate,@maxDate maxDate
