DECLARE @cutoverSessionnumber INT, 	@daysToRetain TINYINT = 7,
		...
		@copiedLogid BIGINT, @maxLogid BIGINT

SELECT @copiedLogid = logid FROM BPC.adf_watermark WHERE tablename = 'BPASessionLog_NonUnicode' 
SELECT @maxLogid = MAX(logid) FROM dbo.BPASessionLog_NonUnicode 
...
CONVERT(NUMERIC(36, 2),(@maxLogid - @copiedLogid)/@ckamt) AS [CurrentDaysToCatchup]
FROM cte_2
